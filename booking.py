...
def check_event(arguments):
    ...
    if final_event:
        ...
        id = final_event['id']
        doctor = final_event['summary']


        return formatted_date, formatted_time, id, doctor
    else:
        return f"Your email is not assigned to any appointment. Please try again."

#---------------BOOKING----------------
        
def appointment_booking(arguments, predermined_doctor = None):
    try:
        provided_date =  str(datetime.strptime(json.loads(arguments)['date'], "%Y-%m-%d").date())
        provided_time = str(datetime.strptime(json.loads(arguments)['time'].replace("PM","").replace("AM","").strip(), "%H:%M:%S").time())
        start_date_time = provided_date + " " + provided_time
...
def appointment_reschedule(arguments):
    ...
    checking_calendar = check_event(arguments)
    appointment_delete(arguments, resechedule= True)
    doctor = json.loads(arguments)['doctor'] or checking_calendar[3]
    appointment_booking(arguments)
...
def appointment_delete(arguments, resechedule = False):
    try:
        provided_date =  str(datetime.strptime(json.loads(arguments)['date'], "%Y-%m-%d").date())
        provided_time = str(datetime.strptime(json.loads(arguments)['time'].replace("PM","").replace("AM","").strip(), "%H:%M:%S").time())
        email_address = json.loads(arguments)['email_address']
        ...
        if provided_date and provided_time and email_address:
            ...
            if start_date_time < datetime.now(timezone):
                ...
                if id:
                    service.events().delete(calendarId='primary', eventId=id).execute()
                    if resechedule == False:
                        return "Appointment deleted successfully."
                    else:
                        return "Appointment deleted successfully. Do you wish to proceed with the reschedule?"
                else:
    ...
    },
    {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                'doctor': {
                    "type" : "string",
                    "description": "If the user wishes to keep the same doctor, this should be assigned as checking_calendar[3], if not a new one should be chosen as doctor"
                },
                 "date": {
                    "type": "string",
                    "format": "date",
...
- If the user wants to reschedule:
    1- Ask for date and time of the original appointment.
    2- Ask for email.
    3- Delete the original appointment 
    4- Ask if the user wishes to mantain the same doctor
    5- If yes yous should call the function appointment_booking with the doctor argument as checking_calendar[3]. If no, then ask for the new doctor
    6- Ask for the date and time of the new appointment
